...
jomleha :
	jomleha M jomle {System.out.println("Rule 21.1 jomleha :
	jomleha M jomle");
	$$ = new EVal();
	...
	|
	jomle {System.out.println("Rule 21.2 jomleha : jomle");
	$$ = new EVal();
	((EVal)$$).nextList = $1.nextList;
	}
...
jomleyeEbarat :
	ebarat NOGHTE_VIRGUL M{
		System.out.println("Rule 22.1 jomleyeEbarat :
	ebarat NOGHTE_VIRGUL M");
		$$ = new EVal();
		((EVal)$$).nextList = $1.nextList;
	}
	|
	NOGHTE_VIRGUL M {System.out.println("Rule 22.2 jomleyeEbarat : NOGHTE_VIRGUL M");
	$$ = new EVal();
		((EVal)$$).nextList = EVal.makeList($2.quad);}

jomleyeEntekhab :
	IF_KW ebarateSade THEN_KW jomle M %prec LOWER_THAN_ELSE_KW {
		System.out.println("Rule 23.1 jomleyeEntekhab :
	IF_KW ebarateSade THEN_KW jomle M %prec LOWER_THAN_ELSE_KW ");
		$$ = new EVal();
		backpatch($2.trueList, $4.quad);
		((EVal)$$).nextList = EVal.merge($2.falseList, $5.nextList);}
	|
	IF_KW ebarateSade THEN_KW M jomle N ELSE_KW M jomle {
	...
	|
	KEY_KW PARANTHESIS_BAZ_KW ebarateSade PARANTHESIS_BASTE_KW N onsoreHalat onsorePishfarz END_KW {System.out.println("Rule 23.3
		jomleyeEntekhab : KEY_KW PARANTHESIS_BAZ_KW ebarateSade PARANTHESIS_BASTE_KW N onsoreHalat onsorePishfarz END_KW ");
		$$ = new EVal();
...
onsoreHalat :
	SWITCH_KW M saved_integer N  DONOGHTE_KW M jomle NOGHTE_VIRGUL { System.out.println("Rule 24.1 onsoreHalat :
	SWITCH_KW M saved_integer N  DONOGHTE_KW M jomle NOGHTE_VIRGUL");
	$$ = new EVal();
	...
	|
	onsoreHalat SWITCH_KW M saved_integer N DONOGHTE_KW M jomle NOGHTE_VIRGUL {System.out.println("Rule 24.2 onsoreHalat : onsoreHalat SWITCH_KW M saved_integer N
	 DONOGHTE_KW M jomle NOGHTE_VIRGUL");
	$$ = new EVal();
...
onsorePishfarz:
	DEFAULT_KW DONOGHTE_KW M jomle N NOGHTE_VIRGUL {System.out.println("Rule 25 onsorePishfarz:
	DEFAULT_KW DONOGHTE_KW M jomle N NOGHTE_VIRGUL");
	$$ = new EVal();
		((EVal)$$).quad = $3.quad;
...
jomleyeTekrar:
	WHILE_KW PARANTHESIS_BAZ_KW M ebarateSade PARANTHESIS_BASTE_KW M jomle {System.out.println("Rule 26 
	jomleyeTekrar: WHILE_KW PARANTHESIS_BAZ_KW M ebarateSade PARANTHESIS_BASTE_KW M jomle ");
	$$ = new EVal();
